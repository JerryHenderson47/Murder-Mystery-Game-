public class Text {

    //Room1 -- the study -- Amnesia Witness
    public static final String studyDescription = "In the mansion's study";
    // Amnesia items:
    //item 1
    public static final String memoryItem1Name = "Shoe";
    public static final String memoryItem1Description = "blood stained runner";
    public static final String memoryItem1Story = "They ran in the door looking for something to clean their shoe ";
    //item 2
    public static final String memoryItem2Name = "Cloth";
    public static final String memoryItem2Description = "Damp dirty and bloody cloth";
    public static final String memoryItem2Story = "They took the cloth and attempted to clean his shoe, however once they saw me they dropped everything";
    //item 3
    public static final String memoryItem3Name = "Pistol";
    public static final String memoryItem3Description = "Old pistol that was still hot";
    public static final String memoryItem3Story = "They pointed the pistol at me implying I stay quite ";
    //item 4
    public static final String memoryItem4Name = "Door Handle";
    public static final String memoryItem4Description = "A broken door handle that looks to have belonged to the study door";
    public static final String memoryItem4Story = "They heard footsteps near the door and rushed to hold the handle of the door, eventually breaking it off the old door";
    // item 5
    public static final String memoryItem5Name = "Hammer";
    public static final String memoryItem5Description = "A new hammer and shiny hammer";
    public static final String memoryItem5Story = "They used the claw of the hammer as a wedge to secure the door until the coast was clear, then they escaped";


    //Room2 -- the bedroom -- Hide and Seek
    public static final String childBedroomDescription = "In a child's bedroom";
    //hiding spots
    //Spot  1
    public static final String hidingSpot1Name = "Wardrobe";
    public static final String hidingSpot1Description = "Large dark oak wardrobe going up to ceiling height with a fancy pattern on the doors";
    // spot 2
    public static final String hidingSpot2Name = "Behind Curtains";
    public static final String hidingSpot2Description = "Ceiling to floor, creatively desingned white curtains";
    //spot 3
    public static final String hidingSpot3Name = "Under Bed";
    public static final String hidingSpot3Description = "Large Double bed with lots of room underneath";
    //spot 4
    public static final String hidingSpot4Name = "Behind door";
    public static final String hidingSpot4Description = "Large wooden door opened";
    // spot 5
    public static final String hidingSpot5Name = "Outside window";
    public static final String hidingSpot5Description = "Large open window with a sill that could easily hold a person";

}
